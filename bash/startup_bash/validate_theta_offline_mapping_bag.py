#!/usr/bin/env python3
"""Validate that a rosbag contains the inputs required for THETA offline road mapping.

ZED/SAM3版 (validate_offline_mapping_bag.py) のTHETA路面マッピング版。
ステレオ・SAM3・カメラ内部パラメータは不要で、Dual Fisheye画像・TF・LiDAR・オドメトリを検証する。
"""
...
REQUIRED_TOPICS = {
    "/theta/dual_fisheye/image_raw/compressed": "THETA Dual Fisheye画像",
    "/tf": "動的TF",
    "/tf_static": "静的TF",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--require-clock",
        action="store_true",
        help="シミュレーション録画として /clock も必須にする",
    )
...
def main() -> int:
    ...
    theta_count = topic_info[theta_topic].message_count if theta_topic in topic_info else 0
    theta_rate = theta_count / duration_sec if duration_sec > 0 else 0.0
    if theta_count and duration_sec >= 4.0 and theta_rate < 0.5:
        failures.append(f"THETA画像レートが低すぎます ({theta_rate:.2f} Hz)")
        print(f"  ✗ THETA画像受信レート: {theta_rate:.2f} Hz (最低0.50 Hz)")
    ...
    required_chains = (
        ...
        ("sirius3/base_footprint", "sirius3/theta_link", "base_footprint → theta_link"),
    )
    ...
    print("=" * 57)
    if failures:
        print("✗ このRosbagはTHETAオフライン路面マッピングに使用できません。")
        for failure in failures:
            print(f"  - {failure}")
        return 1

    print("✓ このRosbagにはTHETA路面マッピングの必須情報が入っています。")
    return 0
